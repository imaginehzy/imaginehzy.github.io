...
首先让走子策略网络（policy network SL或者RL）给出下棋最大概率的几着，然后就是用快速走子进行蒙特卡洛树的搜索，同时也用局面评估网络value network对当前给一个判断。局面评估网络和快速走子的蒙特卡罗树搜索，按照分配两个合起来为一的权重，然后为这一步的值。

到了第N步，程序可能会通过SL展开一个分支，然后又开始对下面的步骤计算权值。

然后对整个分支依步数合并为一个总值，然后看最初哪一步的分支值最高（也就是被访问得最多的，访问得最多胜利可能性越高）就是计算机在比赛现场算出来走子的一步了。
...
darkforest 田渊栋说这步骤只是darkforest第31步值得选择的步骤，阿法狗可能直接忽略了这个分支了。
