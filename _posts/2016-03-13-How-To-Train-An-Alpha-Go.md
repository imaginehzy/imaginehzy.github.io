...
阿法狗始终不是如人在思考，它是固化的机械算法机制罢了。它强大的在于它可以利用上千个CPU，上百个GPU,计算深度上，广度上，做我们做不到的计算探索罢了。

补：后来第四局记者招待会上deepmind负责人哈斯比说，李世石下出神之一手78着后，局面评估网络value network评估局面为胜率70%。在接连9着开始犯傻的时候才发现局面已经变了。可见局面评估网络还是有缺陷的，对局面理解是不完整的。要想起value network是由那三千万局policy network RL自我对弈来的数据，或许正是这种单一性产生的结果
